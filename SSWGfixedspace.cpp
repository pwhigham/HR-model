...
void calc_locn_cumsum(int locn)
{
	...
	for(register int i=0;i<N;++i)
	{
		if (network[locn][i] > 0.0)
		{
			//total += 1; // network[locn][i];  // Use edge weightings for location selection
			total += network[locn][i];  // Use edge weightings for location selection
...
void next_timestep(float r)
{
	...
	// Select parents:  p1 is global, p2 is local 
	...
	// This is the only way to get increased fixation prob. for A
	float savew = network[p1][p1];
	network[p1][p1] = 0.0;
	

	calc_locn_cumsum(p1);  // Setup for random selection of location 
	                       // in the deme of p1
	network[p1][p1] = savew;
