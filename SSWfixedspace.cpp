...
void next_timestep(float r)
{
	...
	int i = randrange(0,(N-1));  // Generate random location -- SSWfixedspace.exe
	...
	// TRY - BUT NOT AT LOCATION <i> 
	
	float savew = network[i][i]; // JUST CHECKING WHAT EFFECT THIS HAS ON STAR....Prob was same as PanWG, TTF
	                               // same as the final WG star ttf.
								   // Result is ttf much faster, but Prob about the same. 
	network[i][i] = 0.0;
	
	calc_locn_cumsum(i);  // Setup for random selection of location 
	                       // in the deme of p1
    network[i][i] = savew;
	
	childloc = select_parent();  // and pick the location for child in deme
	//int childloc = i;  // USE RANDOM LOCATION FOR CHILD.
