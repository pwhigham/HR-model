...
/*
Lookup the fitness for each population member and store
...
Uses the network[locn][N] - if >0 , then include in cumsum running total
*/
...
OUT: Changes pop
*/
void next_generation(float r)
{
	int p1,p2;  // parents
	int tmp;  
	int parent1[2], parent2[2];
	
	// Since panmictic, the cumsum can be calculated once
	...
	for (int i=0;i<N;++i)  // for each location/individual
	{
		...
		nextpop[i][0] = (((float)rand()/(float)(RAND_MAX)) < 0.5)?parent1[0]:parent1[1];
		nextpop[i][1] = (((float)rand()/(float)(RAND_MAX)) < 0.5)?parent2[0]:parent2[1];
	}
...
int main(int argc, char *argv[])
{
	...
	t = (float)Nibeta/(float)N; // See pg 272 HR paper	
